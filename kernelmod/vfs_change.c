...
MODULE_PARM_DESC(hour_shift, "hour shift for displaying /proc/vfs_footmark line item time");

#define MIN_LINE_SIZE	50

static char* get_datetime(long int tv_sec, char *buf)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 6, 0)
	time_t shifted_secs = tv_sec + hour_shift*3600;
#else
	time64_t shifted_secs = tv_sec + hour_shift*3600;
#endif
	struct tm ts;
...
static ssize_t copy_vfs_changes(struct TIMESTRUCT *last, char* buf, size_t size)
{
	...
	list_for_each_entry(vc, &vfs_changes, list) {
		...
		char time[MIN_LINE_SIZE];
		char temp[8192];
		snprintf(temp, sizeof(temp), "%s %s user:%d, app:%s ", get_datetime(vc->ts.tv_sec, time), action_names[vc->action],
				vc->uid, vc->app);
...
// give string format output for debugging purpose (cat /proc/vfs_footmark)
...
static long move_vfs_changes(ioctl_rd_args __user* ira)
{
	...
	list_for_each_safe(p, next, &vfs_changes) {
		...
		char linelog[MAX_LINELOG_LEN] = {0};

		char time[MIN_LINE_SIZE];
		if (vc->dst)
			snprintf(linelog, sizeof(linelog), "%s, action:%s, user:%d, app:%s, filepath:%s -> %s", get_datetime(vc->ts.tv_sec, time), \
				action_names[vc->action], vc->uid, vc->app, vc->src, vc->dst);
		else
			snprintf(linelog, sizeof(linelog), "%s, action:%s, user:%d, app:%s, filepath:%s", get_datetime(vc->ts.tv_sec, time), \
				action_names[vc->action], vc->uid, vc->app, vc->src);

		int this_len = strlen(linelog) + 1;
		if (this_len + total_bytes > kira.size)
			break;

		// src and dst are adjacent when allocated
		memcpy(vfs_changes_buf + total_bytes, linelog, this_len);
...
void vfs_changed(int act, const char* root, const char* src, const char* dst, unsigned int uid)
{
	remove_oldest();
